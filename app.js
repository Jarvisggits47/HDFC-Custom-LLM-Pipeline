...
async function submitTempPasscodeSignIn() {
  ...
  const rawPasscode = document.getElementById("temp-login-passcode")?.value.trim().toUpperCase() || "";

  if (!username || !rawPasscode) {
  ...
  updateSidebarUser();
  bootApp(true);
  logUserAction("TEMP_LOGIN", `${user.name} signed in via Temp Passcode`);
  showToast(`✅ Signed in via Temp Passcode! Welcome back, ${user.name} (${user.empId}).`, "ok");
}
...
async function renderActiveSessionsList() {
  ...
  const masterSession = {
    ...
    device_info: `${isMobile ? "Safari/Chrome on Mobile Device" : "Chrome on Windows 11"} (Primary Master — ${u.name || "Abhi"})`,
    ...
    if (Array.isArray(apiRes) && apiRes.length > 0) {
      apiRes.forEach(s => {
        ...
        if (s && s.status === "active" && !isCurrentMaster) {
          if (!activeList.some(item => (item.token || item.id) === sToken)) {
            activeList.push(s);
          }
        }
      });
  ...
  const localSessions = JSON.parse(localStorage.getItem("hdfc_global_temp_sessions") || "[]");
  localSessions.forEach(s => {
    if (s && s.status === "active") {
      const sToken = s.token || s.id;
      if (!activeList.some(item => (item.token || item.id) === sToken)) {
        activeList.push({
          id: s.id,
          token: sToken,
          ...
          login_type: "temp",
          device_info: s.device || "Mobile Browser (Temp Passcode Override)",
          ip_address: s.ip || "10.42.0.88",
          is_master: false,
          status: "active"
        });
      }
...
async function terminateSession(sessionId) {
  ...
  const localSessions = JSON.parse(localStorage.getItem("hdfc_global_temp_sessions") || "[]");
  const filtered = localSessions.filter(s => s.id !== sessionId && s.token !== sessionId);
